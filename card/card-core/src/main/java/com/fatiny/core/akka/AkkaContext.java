...
final public class AkkaContext {
	...
	private final static long TIME_OUT = 8 * 1000; // 超时
	...
	public static <T> T createTypedActor(Class<T> interfaceCls, Class implementationCls) {
		//Timeout timeout = Timeout.longToTimeout(TIME_OUT);
		Timeout timeout = Timeout.create(Duration.ofMillis(TIME_OUT));
