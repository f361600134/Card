...
//		initService();
	...
	class PipelineWorker extends Thread {
		...
		public void run() {
			...
			for (;;) {
				try {
					...
					GameLog.info("redis pipeline operation occur exception", e);
					ctx = null;
				}
			}
		}
		...
		private void pipelineSync() {
			try {
				...
				GameLog.error("redis连接异常(pipelineSync)", e);
				if (status == NORMAL) {
					status = ABNORMAL;
				}
		...
		private void returnValueAndResp(Object returnObj) {
			...
			try {
				...
				if (reqMsg.getCmd().isReadType()) {
					...
					GameLog.info("从redis加载数据成功:{}", data);
					...
					GameLog.info("redis数据保存结果:{}, 实体:{}", returnObj, po);
					mysqlExecutor.submit(ctx);	// 后续保存到mysql
				}
			} catch (Exception e) {
				GameLog.error("redis响应数据操作发生异常(returnValueAndResp)", e);
				mysqlExecutor.submit(ctx);
			}
		...
		private void doByPipeline(RequestContext ctx) {
			try {
				...
				GameLog.error("redis连接异常(doByPipeline)", e);
				if (status == NORMAL) {
					status = ABNORMAL;
				}
				mysqlExecutor.submit(ctx);
	...
	class DetectionTask implements Runnable {
		...
		public void run() {
			...
			try {
				jedis = connector.getConn();
				if (status == ABNORMAL) {
					GameLog.info("检测redis连接恢复可用:NORMAL");
					status = NORMAL;
				}
			} catch (JedisConnectionException e) {
				if (status == NORMAL) {
					GameLog.error("检测到redis连接异常:ABNORMAL", e);
					status = ABNORMAL;
				}
			} finally {
				connector.closeConn(jedis);
	...
	public void submit(RequestContext ctx) {
		try {
			...
			GameLog.error("redisExecutor异步过程出现异常", e);
		}
	}
